...
public abstract class Config {
	...
	public Config(String name, double version) {
		...
		this.base = BetterAnvils.getInstance();
	...
	public File backup() {
		...
		BetterAnvils.Log("Backing up...");
		
		if(backups.mkdirs() || backups.exists()) {

			BetterAnvils.Log("created");
			...
			if(!this.getConfig().exists()) return null;
				...
				if(success) {
					BetterAnvils.Log("&a" + this.name + ".yml has been backed up!");	
				}else {
					BetterAnvils.Log("&cCouldnt backup!");
				}
			
				return tempConfig;
			
		}else if(!backups.exists()) {
			BetterAnvils.Log("&cCouldnt create backup folder!");
		}
